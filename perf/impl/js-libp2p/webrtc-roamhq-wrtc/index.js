...
export async function main (runServer, serverAddress, transport, uploadBytes, downloadBytes) {
  ...
  if (runServer) {
    // Print all listen multiaddrs to stderr for runner to capture
    // Also write to a file since stderr redirection through perf script is unreliable
    const fs = await import('fs')
    const listenAddrsFile = '/tmp/webrtc-listen-addrs.txt'
    for (const ma of node.getMultiaddrs()) {
      const line = `[LISTEN_ADDR] ${ma.toString()}`
      // eslint-disable-next-line no-console
      console.error(line)
      try {
        fs.appendFileSync(listenAddrsFile, line + '\n')
      } catch (err) {
        console.error('Failed to write listen address to file:', err)
      }
    }
    // Keep server running
  } else {
